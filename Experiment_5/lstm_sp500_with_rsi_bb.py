...
def lade_daten(pfad_basis=None, pfad_rsi=None, pfad_bb=None):
    if pfad_basis is None:
        home_dir = os.path.expanduser('~')
        pfad_basis = os.path.join(home_dir, 'Unternehmenssoftware', 'sp500_data', 'SP500_Index_Historical_Data.csv')

    if pfad_rsi is None:
        pfad_rsi = os.path.join(home_dir, 'Unternehmenssoftware', 'sp500_data', 'RSI',
                                'SP500_Index_Historical_Data_with_RSI.csv')

    if pfad_bb is None:
        pfad_bb = os.path.join(home_dir, 'Unternehmenssoftware', 'sp500_data', 'BollingerBands',
                               'SP500_Index_Historical_Data_with_BB.csv')

    # Basisdaten laden
    if not os.path.exists(pfad_basis):
        raise FileNotFoundError(f"Die Basisdatei wurde nicht gefunden: {pfad_basis}")
    basis_daten = pd.read_csv(pfad_basis)

    # RSI-Daten laden
    if not os.path.exists(pfad_rsi):
        raise FileNotFoundError(f"Die RSI-Datei wurde nicht gefunden: {pfad_rsi}")
    rsi_daten = pd.read_csv(pfad_rsi)

    # BB-Daten laden
    if not os.path.exists(pfad_bb):
        raise FileNotFoundError(f"Die Bollinger Bands-Datei wurde nicht gefunden: {pfad_bb}")
    bb_daten = pd.read_csv(pfad_bb)

    # Sicherstellen, dass die Date-Spalten als Datumswerte interpretiert werden
    basis_daten['Date'] = pd.to_datetime(basis_daten['Date'])
    rsi_daten['Date'] = pd.to_datetime(rsi_daten['Date'])
    bb_daten['Date'] = pd.to_datetime(bb_daten['Date'])

    # Daten nach Datum zusammenführen
    daten = basis_daten.merge(rsi_daten[['Date', 'RSI']], on='Date', how='left')
    daten = daten.merge(bb_daten[['Date', 'BB_Middle', 'BB_Upper', 'BB_Lower']], on='Date', how='left')

    # Fehlende Werte überprüfen und behandeln (z.B. entfernen oder auffüllen)
    daten.dropna(inplace=True)

    return daten


# 2. Daten vorbereiten mit zusätzlichen Features
def bereite_daten_vor(daten,
                      spalten=['Open', 'High', 'Low', 'Close', 'Volume', 'RSI', 'BB_Middle', 'BB_Upper', 'BB_Lower'],
                      sequenz_länge=60):
...
def teste_modell(modell, daten_loader, scaler, gerät):
    ...
    vorhersagen_denorm = scaler.inverse_transform(
        np.hstack((
            ...
            np.zeros((vorhersagen.shape[0], scaler.scale_.shape[0] - 1))  # Nullen für die anderen Spalten
        ))
    ...
    tatsächliche_denorm = scaler.inverse_transform(
        np.hstack((
            ...
            np.zeros((tatsächliche.shape[0], scaler.scale_.shape[0] - 1))
        ))
    )[:, 0]
...
def haupt():
    ...
    daten = lade_daten()
    print("Daten erfolgreich geladen und zusammengeführt.")
    print(f"Gesamte Datenpunkte nach dem Zusammenführen: {len(daten)}")
    ...
    modell = EinfachesLSTM(input_dim=X_train.shape[2])
