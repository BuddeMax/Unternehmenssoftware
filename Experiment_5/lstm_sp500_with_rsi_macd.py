...
def lade_daten(pfad_basis=None, pfad_rsi=None, pfad_macd=None):
    ...
    if pfad_macd is None:
        pfad_macd = os.path.join(home_dir, 'Unternehmenssoftware', 'sp500_data', 'MACD',
                                 'SP500_Index_Historical_Data_with_MACD.csv')
    ...
    # MACD-Daten laden
    if not os.path.exists(pfad_macd):
        raise FileNotFoundError(f"Die MACD-Datei wurde nicht gefunden: {pfad_macd}")
    macd_daten = pd.read_csv(pfad_macd)
    ...
    macd_daten['Date'] = pd.to_datetime(macd_daten['Date'])

    # Daten nach Datum zusammenführen
    daten = basis_daten.merge(rsi_daten[['Date', 'RSI']], on='Date', how='left')
    daten = daten.merge(macd_daten[['Date', 'MACD', 'Signal_Line']], on='Date', how='left')
...
def bereite_daten_vor(daten, spalten=['Open', 'High', 'Low', 'Close', 'Volume', 'RSI', 'MACD', 'Signal_Line'],
                      sequenz_länge=60):
