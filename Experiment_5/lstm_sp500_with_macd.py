...
def lade_daten(pfad=None):
    """
    Lädt die historischen S&P 500 Daten mit MACD aus einer CSV-Datei.
    ...
    if pfad is None:
        ...
        pfad = os.path.join(home_dir,
                            'Unternehmenssoftware/shared_resources/sp500_data/MACD/SP500_Index_Historical_Data_with_MACD.csv')
...
# 2. Daten vorbereiten
def bereite_daten_vor(daten, spalten=['Open', 'High', 'Low', 'Close', 'Volume', 'MACD', 'Signal_Line'],
                      sequenz_länge=60):
    """
    ...
    """
    # Fehlende Werte überprüfen und ggf. entfernen
    if daten[spalten].isnull().values.any():
        daten = daten.dropna(subset=spalten)
...
class EinfachesLSTM(nn.Module):
    ...
    def __init__(self, input_dim=7, hidden_dim=50, output_dim=1, num_layers=2):
        """
